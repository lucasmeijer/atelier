...
export function workspaceWarnings(entry: WorkspaceEntry, project: ProjectConfiguration | undefined): WorkspaceWarning[] {
  ...
  if (isGitProjectInit(entry.init)) {
    ...
    // Explicit settings snapshots (onboarding and experiments) deliberately differ from saved settings.
    // Older workspaces have no fingerprint; do not claim to know whether their settings changed.
    if (!entry.init.settings && entry.init.configurationFingerprint && entry.init.configurationFingerprint !== configuration.configurationFingerprint) {
