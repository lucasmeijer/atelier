...
export class SubagentTranscript implements AgentDelegationTranscript {
  ...
  private communicationItems(): Extract<TranscriptItem, { type: "extension" }>[] {
    ...
    return state.messages.filter((message) => (message.to === this.conversationId || inheritedIds.has(message.id)) && ["task", "message", "completion"].includes(message.kind)).map((message) => communicationItem({
