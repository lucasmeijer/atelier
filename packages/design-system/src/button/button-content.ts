...
export function buttonContentHtml(content: ButtonContent): string {
  if (content.kind === "caption") return `${content.iconHtml ? `<span class="button__icon" aria-hidden="true">${content.iconHtml}</span>` : ""}<span class="button__caption">${escapeHtml(content.caption)}</span>`;
