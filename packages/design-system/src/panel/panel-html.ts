...
export function panelHtml(options: PanelOptions): string {
  ...
  return `<${element.tag} class="${escapeHtml(classNames("panel", element.className))}"${attributesHtml(element.attributesHtml)}><header class="panel__header">${options.headerHtml}</header><div class="${escapeHtml(classNames("panel__body", options.bodyClassName))}">${options.bodyHtml}</div>${footer}</${element.tag}>`;
}
