...
function page(theme: string): string {
  const stepHtml = steps.map((step) => {
    ...
    return `<li class="status-list__item"${presentation.rowAttributes}><span class="status-list__marker"${presentation.markerAttributes}>${presentation.marker}</span><div class="update-helper__step">${step.label}${step.log ? `<pre>${escapeHtml(step.log)}</pre>` : ""}</div></li>`;
  }).join("");
  return `<!DOCTYPE html><html data-theme="${escapeHtml(theme || "nord")}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Updating Atelier</title><link rel="stylesheet" href="/design-system.css"><style>*{box-sizing:border-box}body{min-height:100vh;display:grid;place-items:center;margin:0;padding:32px 24px;background:var(--bg);color:var(--text);font:var(--text-body)/var(--leading-standard) var(--font-sans)}.update-helper{width:min(620px,100%)}.update-helper>p{margin:0 0 24px}.update-helper__step{min-width:0;flex:1}.update-helper pre{max-height:220px;margin:8px 0 0;padding:12px;overflow:auto;border:1px solid var(--line);border-radius:var(--radius-surface);background:var(--bg);color:var(--text);font:inherit;white-space:pre-wrap}</style><script>setInterval(async()=>{const r=await fetch('/state');const s=await r.json();if(s.redirect) location.href=s.redirect; else location.reload();},1200)</script></head><body><main class="update-helper"><p>${failed ? "The update needs attention. The logs below should help you recover over SSH." : "Atelier is restarting. This usually takes a few seconds."}</p><ol class="status-list">${stepHtml}</ol></main></body></html>`;
}
