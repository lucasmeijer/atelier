import { commandSignal, waitForCommand } from "./command-scope.ts";
...
export function createKeyedOperationQueue() {
  ...
  return async function run<Result>(key: string, operation: () => Promise<Result>): Promise<Result> {
    ...
    const result = waitForCommand(previous).then(() => { signal?.throwIfAborted(); return operation(); });
    const settled = previous.then(() => result).then(() => undefined, () => undefined);
