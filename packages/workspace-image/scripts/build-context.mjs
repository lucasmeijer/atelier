...
// Build only the workspace executable and its dependencies, without shipping Go.
const snapshotterSource = join(packagesDir, "docker-snapshotter");
for (const name of await workspaceSnapshotterInputs(snapshotterSource)) {
  await mkdir(dirname(join(outDir, "snapshotter", name)), { recursive: true });
