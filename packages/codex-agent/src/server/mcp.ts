import { prepareAgentMcp } from "@atelier/agent/server";
import { shellQuote } from "@atelier/core";
import { execWorkspaceShell } from "@atelier/workspace";

export async function prepareCodexMcp(workspaceId: string, sessionId: string): Promise<Record<string, string>> {
  const mcp = await prepareAgentMcp(workspaceId, sessionId);
