...
test("workspace creation persists recency without changing workspace configuration", async () => {
  ...
  await recordWorkspaceCreation(project.id, 1000, file);
...
test("projectless creation persists recency independently of projects", async () => {
  const { project } = await addProject("https://github.com/org/repo.git", file);
  await Promise.all([2000, 3000, 1000].map((at) => recordWorkspaceCreation(undefined, at, file)));
  const result = await listProjects(file);
  expect(result.lastProjectlessWorkspaceCreatedAt).toBe(3000);
  expect(result.projects[0]?.lastWorkspaceCreatedAt).toBeUndefined();
  await recordWorkspaceCreation(project.id, 4000, file);
  expect((await listProjects(file)).lastProjectlessWorkspaceCreatedAt).toBe(3000);
});
...
test("concurrent creations retain the latest timestamp and leave other projects untouched", async () => {
  ...
  await Promise.all([2000, 3000, 1000].map((at) => recordWorkspaceCreation(project.id, at, file)));
