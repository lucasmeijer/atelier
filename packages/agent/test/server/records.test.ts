...
describe("recordsFromSessionEntries", () => {
  test("maps pi session message entries to transcript records", () => {
    const entries = [
      ...
      { type: "message", id: "e3", parentId: "e2", timestamp: "2026-06-10T10:00:06Z", message: { role: "toolResult", toolCallId: "c1", toolName: "bash", content: [{ type: "text", text: "out" }], details: { displayAnsi: "\x1b[31mout\x1b[0m" }, isError: false, timestamp: 1760000006000 } },
    ...
    expect(records[2]).toMatchObject({ kind: "toolResult", callId: "c1", text: "out", details: { displayAnsi: "\x1b[31mout\x1b[0m" } });
