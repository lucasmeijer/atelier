...
const reportedUsageWindowSchema = {
  ...
  properties: {
    ...
    usedPercent: { type: "number" }, durationSeconds: { type: "integer" }, resetsAt: { type: "string", format: "date-time" },
  },
};
const providerUsageSchema = {
  ...
  properties: {
    ...
    reported: { type: ["object", "null"], properties: {
      plan: { type: "string" }, checkedAt: { type: "string", format: "date-time" }, allowed: { type: ["boolean", "null"] }, limitReached: { type: ["boolean", "null"] },
    ...
    windows: { type: "array", items: { type: "object", properties: {
      ...
      measured: { ...measuredUsageSchema, type: ["object", "null"], description: "All provider tokens in the reported interval, not feature-filtered; null if the interval has not started. Not convertible to subscription percentages." },
      timing: { type: "object", description: "Linear pacing reference at reported.checkedAt. Start is inferred from reset minus duration, not explicitly reported by OpenAI.", properties: {
        startsAt: { type: "string", format: "date-time" }, elapsedPercent: { type: "number", minimum: 0, maximum: 100 },
        ...
        state: { enum: ["not-started", "active", "reset-due"] }, paceDifferencePoints: { type: ["number", "null"], description: "Usage minus elapsed-time percentage, in percentage points. Positive is above linear pace. Null outside an active window." },
      } },
    } } },
  },
};
...
export const usageOpenApiPaths = {
  "/usage": { get: {
    ...
    description: "HTML opens the Usage dialog in the Atelier shell. JSON includes provider-reported subscription windows and installation-local token measurements. Currently supports OpenAI Codex only. Provider failures are explicit per-provider errors and do not hide local measurements.",
