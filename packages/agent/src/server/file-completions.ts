...
export function renderFileCompletionMenu(completions: readonly FileCompletion[]): string {
  ...
  return `<div class="agent-completion-menu" role="listbox" aria-label="Files and directories">${completions.map((completion, index) => {
    ...
    return `<button type="button" class="agent-completion-option agent-file-option${index === 0 ? " active" : ""}" role="option" aria-selected="${index === 0 ? "true" : "false"}" data-completion-kind="file" data-file-path="${escapeHtml(path)}" data-file-directory="${completion.directory}">
