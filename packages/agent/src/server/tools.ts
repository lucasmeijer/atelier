...
function createPresentTool(workspaceId: string, options: WorkspaceAgentToolOptions): ToolDefinition<any, any> | undefined {
  ...
  return defineTool({
    name: "present",
    label: "Present",
    description: "Present one primary interactive surface to the user in Atelier. Use this when there is one main thing the user should look at or interact with while evaluating your work. Atelier will place the chosen surface in the preview area. Calling this again should update or replace the primary presentation rather than adding multiple competing presentations. Only use this tool for interactive surfaces that need explicit presentation, as described by the available presentation kinds. Do not use this tool for static or inline artifacts. Images, videos, SVGs, and HTML files are already automatically visible to the user when you reference them with an Atelier embed URL in Markdown image syntax, for example: ![](atelier-embed:/work/app/screenshot.png) or ![](atelier-embed:/work/app/demo.html). For ordinary screenshots, videos, generated HTML explanations, or file previews, prefer the embed syntax instead of this tool. " + presenters.map((presenter) => `${presenter.kind}: ${presenter.description}`).join(" "),
