...
export function createWorkspacePresentationStore(options: WorkspacePresentationStoreOptions): WorkspacePresentationStore {
  ...
  function parseReference(workspaceId: string, value: unknown, stored = false): WorkspaceWorkViewReference {
    if (!isObject(value)) throw referenceError(workspaceId, "reference must be an object", stored);
  ...
  function parse(workspaceId: string, value: unknown): StoredPresentation {
    if (!isObject(value)) throw presentationError(workspaceId, "expected version 1 state");
    const version = field(value, "version");
    const nextAttentionSequence = field(value, "nextAttentionSequence");
    const storedWorkViews = field(value, "workViews");
    ...
    const workViews = storedWorkViews.map((entry, index) => {
      if (!isObject(entry)) throw presentationError(workspaceId, `Work view ${index} must be an object`);
      const reference = parseReference(workspaceId, field(entry, "reference"), true);
      const attentionSequence = field(entry, "attentionSequence");
      ...
      return { reference, ...(attentionSequence === undefined ? {} : { attentionSequence: Number(attentionSequence) }) };
    });
  ...
  return {
    ...
    async listWorkViews(workspaceId) {
      return await serialized(workspaceId, async () => {
        ...
        return state.workViews.map((view) => ({
          ...
          attention: view.attentionSequence !== undefined,
          ...(view.attentionSequence === undefined ? {} : { attentionSequence: view.attentionSequence }),
        }));
      });
    },
    ...
    async acknowledgeAttention(workspaceId, inputReference) {
      await serialized(workspaceId, async () => {
        ...
        const view = state.workViews.find((candidate) => identity(candidate.reference) === identity(reference));
        if (!view) throw new AtelierCoreError("work_view_not_found", `Work view is not open: ${identity(reference)}`);
        if (view.attentionSequence === undefined) throw new AtelierCoreError("attention_not_requested", `Work view has no Attention request: ${identity(reference)}`);
